#!/usr/bin/python3
'''Contains the API's view for places_amenities.'''
...
def handle_places_amenities(place_id=None, amenity_id=None):
    '''The method handler for the endpoint related to places.
    '''
    handlers = {
...
def get_place_amenities(place_id=None, amenity_id=None):
    ...
    place identified by the given ID.
    '''
    if place_id:
...
def remove_place_amenity(place_id=None, amenity_id=None):
    '''Disassociates an amenity with a given
    ID from a place with a given ID.
    '''
...
def add_place_amenity(place_id=None, amenity_id=None):
    '''Associates an amenity with a given ID to a place with a given ID.
    '''
    if place_id and amenity_id:
        place = storage.get(Place, place_id)
        if not place:
